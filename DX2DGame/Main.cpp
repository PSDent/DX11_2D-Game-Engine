...
ID3D11ShaderResourceView*			g_pTexture_2 = NULL;
...
HRESULT InitDevice()
{
 ...
 ////   // Load the Texture
   hr = D3DX11CreateShaderResourceViewFromFile( g_pd3dDevice, L"Mushroom.png", NULL, NULL, &g_pTextureRV, NULL );
   hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"seafloor.dds", NULL, NULL, &g_pTexture_2, NULL);
   if (FAILED(hr))
   {
	   MessageBox(NULL, L"Failed to Create ShaderResource", L"ERROR", MB_OK);
	   return hr;
   }
...
void Render()
{
	// 카메라 이동 해보기, GameObject 파생 클래스 구현 
